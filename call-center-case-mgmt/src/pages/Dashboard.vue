    ...
    <div class="sidebar" :class="{ collapsed: isSidebarCollapsed, 'mobile-open': mobileOpen }" id="sidebar">
      ...
      <div class="sidebar-content">
        ...
        <div class="nav-section">
          <router-link to="/dashboard" class="nav-item" :class="{ active: $route.path === '/dashboard' }">
            <div class="nav-icon">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M3 9L12 2L21 9V20C21 20.5304 20.7893 21 20.4142 21.4142C20.0391 21.7893 19.5304 22 19 22H5C4.46957 22 3.96086 21.7893 3.58579 21.4142C3.21071 21.0391 3 20.5304 3 20V9Z"
          ...
          <router-link to="/superadmin" class="nav-item" v-if="userRole === 'super-admin'"
            :class="{ active: $route.path === '/super-admin-dashboard' }">
            <div class="nav-icon">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M12 22S8 18 8 12V5L12 2L16 5V12C16 18 12 22 12 22Z" stroke="currentColor" stroke-width="2"
                  stroke-linecap="round" stroke-linejoin="round" />
        ...
        <div class="sidebar-bottom">
          ...
          <div class="button-container">
            <button class="join-queue-btn" @click="toggleQueue" :class="{ 'in-queue': isInQueue }">
          ...
          <div class="chart-card">
            ...
            <div class="chart-placeholder">
              <div class="line-chart">
                ...
                <div class="y-label" style="bottom: 20px;">0</div>
                <div class="y-label" style="bottom: 77px;">10</div>
                <div class="y-label" style="bottom: 134px;">20</div>
                <div class="y-label" style="bottom: 191px;">30</div>
                <div class="y-label" style="bottom: 248px;">40</div>

                <div class="chart-axis"></div>

                <div class="chart-line">
                  <div v-for="(point, index) in chartData" :key="index" class="chart-point">
                    <div class="point-dot" :style="{ bottom: point.height + 'px' }"></div>
                    <div class="point-line" :style="{ height: point.height + 'px', bottom: '0' }"></div>
  ...
  export default {
    setup() {
      ...
      const callStatusText = computed(() => {
        if (activeCall.value) return 'On Call'
        if (incomingCall.value) return 'Incoming Call'
        return isInQueue.value ? 'Ready for Calls' : 'Offline'
      })
      ...
      const toggleMute = () => {
        ...
        // Implement mute functionality with SIP client
      }
      ...
      const transferCallToTarget = () => {
        if (transferTarget.value) {
          transferCall(transferTarget.value)
      ...
      onMounted(async () => {
        const initializeSipClient = async () => {
          try {
            // 1. Safely retrieve and parse stored SIP details
            let sipDetails = {};
            try {
              const storedData = localStorage.getItem('sipConnectionDetails');
              sipDetails = storedData ? JSON.parse(storedData) : {};
            } catch (e) {
              console.error('Failed to parse stored SIP details:', e);
              return;
            }

            console.log('SIP Details:', sipDetails);

            // 2. Validate required fields with better error reporting
            const requiredFields = ['uri', 'password', 'websocketURL'];
            const missingFields = requiredFields.filter(field => !sipDetails[field]);

            if (missingFields.length > 0) {
              console.error('Missing required SIP fields:', missingFields);
              return;
            }

            // 3. Initialize SIP connection with error handling
            try {
              await initSIP(sipDetails);
            } catch (error) {
              console.error('SIP initialization failed:', error);
              return;
            }

            // 4. Set up event listeners with cleanup consideration
            const eventHandlers = {
              onRegistered: () => {
              ...
              onIncomingCall: (session) => {
                incomingCall.value = {
                  ...
                  callerNumber: session.remote_identity.uri.user || 'Unknown'
                };
              },
              onCallAnswered: () => {
            ...
            console.error('Unexpected error during SIP initialization:', error);
          }
        };

        // Usage
        const cleanupSip = await initializeSipClient();

        // Remember to call cleanupSip() when component unmounts
        // Apply saved theme
...
<style>
  ...
  * {
  ...
  /* Enhanced tablet and mobile sidebar scrolling */
  @media (max-width: 1024px) {
    .sidebar-content {
      overflow-y: auto;
      overflow-x: hidden;
  ...
  /* Compact mode for smaller heights on tablets */
  ...
  /* Extra compact mode for very small heights on mobile */
